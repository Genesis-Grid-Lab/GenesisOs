...
namespace UE {
	...
	void Scene::OnRuntimeStart(){
		...
		GroupEntity<BoxShapeComponent>([this] (auto entity, auto& comp, auto& transform, auto id){
			...
			if (result.HasError()) {
				UE_CORE_ERROR("Failed to create offset shape: {0}", result.GetError());
				return;
			}
			...
			{
				UE_CORE_WARN("Error creating shape: {0}",
		...
		GroupEntity<CharacterComponent>([this] (auto entity, auto& comp, auto& transform, auto id){
			...
			// UE_CORE_WARN("extent {0} {1} {2}", scaledHalfExtents.x, scaledHalfExtents.y, scaledHalfExtents.z);
			// UE_CORE_WARN("offset {0}", joltCenterOffset);
			if(entity.template HasComponent<BoxShapeComponent>()){
			...
			// UE_CORE_WARN("Original Pos: {0}", transform.Translation);
			// UE_CORE_WARN("Final Pos: {0}", glm::vec3(characterPos.GetX(), characterPos.GetY(),characterPos.GetZ()));		
	...
	void Scene::PhysicsUpdate(float dt){
		...
		ViewEntity<Entity, CharacterComponent>([this, &dt] (auto entity, auto& comp){
			...
			const JPH::RVec3 position{
				body_interface.GetCenterOfMassPosition(comp.ID)};
			const JPH::Vec3 velocity{body_interface.GetLinearVelocity(comp.ID)};
			// UE_CORE_INFO("Character Step {0}: Position = ({:.{}f}, {:.{}f}, "
			// 			"{:.{}f}), Velocity = ({:.{}f}, {:.{}f}, {:.{}f})\n",
			// 			m_Physics3D._step,
			// 			position.GetX(),
			// 			2,
